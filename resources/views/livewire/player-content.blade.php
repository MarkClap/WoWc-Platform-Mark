<main class="flex flex-row flex-grow relative h-full w-full max-h-full max-w-full">
    <aside id="menu"
        class="z-20 text-white absolute bottom-0 left-0 duration-500 p-2 w-52 h-full max-h-full text-center bg-neutral-700 shadow">
        <div class="text-lg flex flex-row justify-between h-full w-full max-h-full max-w-full ">
            <ul class="flex flex-col gap-2 h-full w-full max-h-full max-w-full justify-between board-sidebar-content">
                <div>
                    <li>
                        <button wire:click="updateContent('main.player.character')">
                            <div>
                                <span class="icon-[heroicons--user-solid]"></span>
                            </div>
                            <span>
                                Personajes
                            </span>
                        </button>
                    </li>

                    <li>
                        <button wire:click="updateContent('main.player.quests')">
                            <div>
                                <span class="icon-[mingcute--task-2-fill]"></span>
                            </div>
                            <span>
                                Misiones
                            </span>
                        </button>
                    </li>

                    <li tabindex="0" class="collapse collapse-arrow">
                        <div class="collapse-title">
                            <div>
                                <div>
                                    <span class="icon-[game-icons--vertical-banner]"></span>
                                </div>
                                <span>Gremio</span>
                            </div>
                        </div>
                        <ul class="collapse-content">
                            <li>
                                <button wire:click="updateContent('main.player.members')">
                                    <div>
                                        <span class="icon-[heroicons--user-group-solid]"></span>
                                    </div>
                                    <span>Miembros</span>
                                </button>
                            </li>

                            <li>
                                <button wire:click="updateContent('main.player.groups')">
                                    <div>
                                        <span class="icon-[heroicons--users-solid]"></span>
                                    </div>
                                    <span>Grupo</span>
                                </button>
                            </li>
                        </ul>
                    </li>
                </div>
                <div>
                    <li>
                        <button>
                            <div>
                                <span class="icon-[material-symbols--exit-to-app]"></span>
                            </div>
                            <span>
                                Salida
                            </span>
                        </button>
                    </li>
                </div>
            </ul>
        </div>
    </aside>

    @component($content)
    @endcomponent

</main>
